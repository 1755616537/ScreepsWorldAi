...
import { reportWrongExit } from '../report/report.wrongExit'; //异常退出 - 报告模块
import { taskSpawn } from '../task/task.spawn'; //队列孵化 - 任务模块
import { taskNewRoomBuild } from '../task/task.newRoomBuild'; //新基地建设 - 任务模块

export const queueTask_W49S53 = {
    /**
    * 'W49S53'任务-任务队列模块
    */
    run: function() {
        ...
        //200能量 - 单兵建设爬爬组件
        var moduleNewRoomBuilder = [WORK,CARRY,MOVE,MOVE];
        ...
        //#1-1 单兵建设1
        if(taskNewRoomBuild.run('1-1', 3, nameRoom, moduleNewRoomBuilder, 1, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('#1-1 单兵建设1');
            
        //#1-2 单兵建设2
        if(taskNewRoomBuild.run('1-2', 3, nameRoom, moduleNewRoomBuilder, 0, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('#1-2 单兵建设2');
...
//module.exports = queueTask_W39N53;