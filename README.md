...
> 当（[./](./)）目录下没有（[.secret.json](./.secret.json)）文件时，可以从目录（[./others](./others)）下，
复制（[Tocken例子.secret.json](others/Tocken例子.secret.json)）改名（[.secret.json](./.secret.json)），
配置内容修改成，游戏中的[Tocken](docs/authenticating.md)（[main.token]()），本地游戏代码目录路径（[local.copyPath]()）。

[Tocken生成方法](docs/authenticating.md)
...
```
[.secret.json文件 详情说明](docs/deploy-destinations.md)

## 测试
[详情说明](docs/testing.md)
### 单元测试
### 集成测试
